...
class Application:
    ...
    def add_task_to_treeview(self,task_text, priority):
        """向Treeview中添加任务并设置颜色"""
        self.item_id = self.treeview.insert('', 'end', values=(task_text, priority))

        # 根据优先级设置行颜色
        self.color_info = self.PRIORITY_INFO.get(priority, {'color': '#ffffff'})
        self.color = self.color_info['color']
        self.treeview.tag_configure(priority, background=self.color)
        self.treeview.item(self.item_id, tags=(priority,))
    #添加任务1
    def selected(self):
        self.task_text = self.task_create.get('1.0', END).strip()
        ...
        # 添加到Treeview
        self.add_task_to_treeview(self.task_text, self.value)
    ...
    def create(self):
        ...
        self.task_created = Button(self.window, text='创建', padding=(15, 5), command=self.selected, bootstyle='success')
        self.delete = Button(self.window, text='删除', padding=(15, 5), command=self.delete_selected, bootstyle='danger')
        ...
        self.volume_scale = Scale(self.window, from_=0, to=100, orient='vertical', length=180, bg="#f0f0f0", command=self.volume)
        self.volume_scale.set(30)  # 默认音量
        self.attributes = Button(self.window,text='取消置顶', padding=(10, 2), command=self.attribute,  bootstyle='warning')
        self.l1 = Label(self.window, text='0%')
        self.l2 = Label(self.window, text='100%')

        # 添加排序按钮
        self.sort_priority_btn = Button(self.window, text='按优先级排序', padding=(10, 2), command=self.sort_by_priority, bootstyle='warning')
        self.sort_task_btn = Button(self.window, text='按名称排序', padding=(10, 2), command=self.sort_by_task, bootstyle='warning')
